...
from openfold.data.xl_subsampling import sample_xl
from openfold.data.msa_subsampling import get_eff, subsample_msa, subsample_msa_sequentially, subsample_msa_random
...
class OpenFoldSingleDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        crosslink_dir: Optional[str] = None,
        ...
        self.crosslink_dir = crosslink_dir
        self.config = config
        ...
        self._chain_ids = []
        for i, target, xl in np.loadtxt(mapping_path, dtype=str):
            i = int(i)
            self._chain_id_to_idx_dict[i] = target
            self.crosslinks[i] = torch.load(self.crosslink_dir + '/' + xl)
    ...
    def __getitem__(self, idx):
        ...
        if(self.feature_dir is None):
            ...
            if self.mode == 'train':
                tp = xl['tp']
                fp = xl['fp']
                xl, xl_grouping, real = sample_xl(tp,fp,seq,ambiguous=False)
            else:
        ...
        if(self.training_mode):
            ...
            if(self.val_data_dir is not None):
                self.eval_dataset = dataset_gen(
                    ...
                    crosslink_dir='/localscratch/openfold_validation_xl_static',
