...
class AlphaFold(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.xl_grouping_embedder = XLGroupingEmbedder(
            **config["xl_embedder"],
        )
    ...
    def iteration(self, feats, m_1_prev, z_prev, x_prev, _recycle=True):
        ...
        if not self.config.xl_embedder.distograms:
            z = z + self.xl_embedder(feats["xl"])
            z = z + self.xl_grouping_embedder(feats["xl_grouping"])
        else:
            z = z + self.xl_embedder(feats["xl"] * 1000) # scale up to force signal
